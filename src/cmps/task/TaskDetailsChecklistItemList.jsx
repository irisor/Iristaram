...
export function TaskDetailsChecklistItemList({ checklist, onChangeProgress }) {
	...
	useEffect(() => {
		if (JSON.stringify(checklistForm) !== JSON.stringify(initialChecklistForm)) {
			const newTask = { ...currentTask, checklists: currentTask.checklists.map(newChecklist => {
	...
	return (
		<form onSubmit={ev => ev.preventDefault()} className="task-details-checklist-list" >
			{checklist.checkItems.map(checkItem => 
				<TaskDetailsChecklistItemPreview key={checkItem.id} checkItem={checkItem} onChange={ev => onChange(ev)}/>
			)}
		</form>
	)
}