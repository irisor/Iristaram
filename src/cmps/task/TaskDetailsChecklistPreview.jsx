...
export function TaskDetailsChecklistPreview({ checklist, triggerResetAll, resetAll }) {
	...
	async function deleteChecklist({target}) {
		await updateTask(board._id, groupId, { ...currentTask, checklists: currentTask.checklists.filter(cl => cl.id !== target.closest('.task-details-checklist').id) })
	}
	...
	function onAddCheckItem(boardId, groupId) {
		return function (checkItemTitle) {
			...
			const newCheckItem = {title: checkItemTitle, id: utilService.makeId(), checked: false}
			try {
	...
	return (
		<section className="task-details-checklist task-details-main-item" key={checklist.id} id={checklist.id}>
			<section className="task-details-main-item-header">
				...
				<button className="btn" onClick={ev =>deleteChecklist(ev)}>Delete</button>
			</section>
			...
			<CreateItem onAddItem={onAddCheckItem(board._id, groupId)} initialBtnLabel='Add an item' 
				...
				resetAll={resetAll} triggerResetAll={triggerResetAll} thisId={checklist.id}/>
		</section>
	)
}